...
namespace Trajectories
{
    ...
    internal abstract class AerodynamicModel
    {
        ...
        {
            cachedForces = new AeroForceCache(10000d, Math.PI, GameDataCache.VesselBodyInfo.AtmosphereDepth, 32, 33, 32, this);
        ...
        {
            double altitudeAboveSea = bodySpacePosition.magnitude - GameDataCache.VesselBodyInfo.Radius;
            if (altitudeAboveSea > GameDataCache.VesselBodyInfo.AtmosphereDepth)
            {
        ...
        internal Vector3d ComputeForces(double altitude, Vector3d airVelocity, Vector3d vup, double angleOfAttack)
        {
            ...
            if (!GameDataCache.VesselBodyInfo.HasAtmosphere || altitude >= GameDataCache.VesselBodyInfo.AtmosphereDepth)
                return Vector3d.zero;
